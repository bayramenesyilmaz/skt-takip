...
export class LocalRepository implements IRepository {
  ...
  async updateOrganization(orgId: string, data: Partial<Organization>): Promise<void> {
    ...
    if (idx >= 0) {
      orgs[idx] = { ...orgs[idx], ...data, updated_at: now() }
      write(KEYS.org, orgs)
    }
  }

  // ============================================================
  // BRANCHES (no-op for local)
  // ============================================================
  async getBranches(_orgId: string): Promise<Branch[]> {
  ...
  async createBranch(data: Partial<Branch>): Promise<Branch> {
    ...
    const branches = read<Branch>(KEYS.branches)
  ...
  async updateBranch(branchId: string, data: Partial<Branch>): Promise<void> {
    ...
    if (idx >= 0) {
      branches[idx] = { ...branches[idx], ...data, updated_at: now() }
      write(KEYS.branches, branches)
    }
  }
  ...
  async deleteBranch(branchId: string): Promise<void> {
    write(
      KEYS.branches,
      read<Branch>(KEYS.branches).filter((b) => b.id !== branchId),
    )
  }

  // ============================================================
  // PROFILES (no-op for local)
  // ============================================================
  async getProfiles(_orgId: string): Promise<Profile[]> {
  ...
  async deleteProfile(profileId: string): Promise<void> {
    write(
      KEYS.profiles,
      read<Profile>(KEYS.profiles).filter((p) => p.id !== profileId),
    )
  }
  ...
  async updateLocation(locationId: string, data: Partial<Location>): Promise<void> {
    ...
    if (idx >= 0) {
      locations[idx] = { ...locations[idx], ...data, updated_at: now() }
      write(KEYS.locations, locations)
    }
  }
  ...
  async deleteLocation(locationId: string): Promise<void> {
    ...
    write(
      KEYS.locations,
      locations.filter((l) => l.id !== locationId && l.parent_id !== locationId),
    )
  }
  ...
  async createBrand(data: Partial<Brand>): Promise<Brand> {
    const brand: Brand = {
      ...
      branch_id: data.branch_id,
      name: data.name || '',
      return_accepts: data.return_accepts ?? true,
      created_at: now(),
      updated_at: now(),
    }
  ...
  async updateBrand(brandId: string, data: Partial<Brand>): Promise<void> {
    ...
    if (idx >= 0) {
      brands[idx] = { ...brands[idx], ...data, updated_at: now() }
      write(KEYS.brands, brands)
    }
  }
  ...
  async deleteBrand(brandId: string): Promise<void> {
    write(
      KEYS.brands,
      read<Brand>(KEYS.brands).filter((b) => b.id !== brandId),
    )
  }
    ...
    return products
      .map((p) => {
        ...
        return {
          ...
          brand,
          stock_items: items,
          total_quantity: items.reduce((sum, s) => sum + s.quantity, 0),
        } as ProductWithStock
      })
  ...
  async deleteProduct(productId: string): Promise<void> {
    write(
      KEYS.products,
      read<Product>(KEYS.products).filter((p) => p.id !== productId),
    )
    write(
      KEYS.stock,
      read<StockItem>(KEYS.stock).filter((s) => s.product_id !== productId),
    )
  }

  async bulkCreateProducts(items: Partial<Product>[]): Promise<Product[]> {
    const products = read<Product>(KEYS.products)
    const newProducts: Product[] = items.map((data) => ({
      id: uid(),
      org_id: data.org_id || '',
      branch_id: data.branch_id,
      name: data.name || '',
  ...
  async createStockItem(data: Partial<StockItem>): Promise<StockItem> {
    const item: StockItem = {
      ...
      expiry_date: data.expiry_date || now(),
  ...
  async updateStockItem(stockId: string, data: Partial<StockItem>): Promise<void> {
    ...
    if (idx >= 0) {
      stock[idx] = { ...stock[idx], ...data, updated_at: now() }
      write(KEYS.stock, stock)
    }
  }
  ...
  async deleteStockItem(stockId: string): Promise<void> {
    write(
      KEYS.stock,
      read<StockItem>(KEYS.stock).filter((s) => s.id !== stockId),
    )
  }

  async bulkCreateStockItems(items: Partial<StockItem>[]): Promise<StockItem[]> {
    const stock = read<StockItem>(KEYS.stock)
    const newItems: StockItem[] = items.map((data) => ({
      id: uid(),
      org_id: data.org_id || '',
      branch_id: data.branch_id,
      product_id: data.product_id || '',
    ...
    write(KEYS.stock, [...stock, ...newItems])
    return newItems
  }
  ...
  async createImportTemplate(data: Partial<ImportTemplate>): Promise<ImportTemplate> {
    ...
    const templates = read<ImportTemplate>(KEYS.templates)
  ...
  async deleteImportTemplate(templateId: string): Promise<void> {
    write(
      KEYS.templates,
      read<ImportTemplate>(KEYS.templates).filter((t) => t.id !== templateId),
    )
  }
}
