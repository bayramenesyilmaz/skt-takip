...
export class SupabaseRepository implements IRepository {
  ...
  async getBranches(orgId: string): Promise<Branch[]> {
    const { data, error } = await supabase.from('branches').select('*').eq('org_id', orgId).order('created_at', { ascending: true })
    if (error) throw error
  ...
  async getProfiles(orgId: string): Promise<Profile[]> {
    const { data, error } = await supabase.from('profiles').select('*').eq('org_id', orgId).order('created_at', { ascending: true })
    if (error) throw error
  ...
  async getProducts(orgId: string, branchId?: string): Promise<ProductWithStock[]> {
    let query = supabase.from('products').select('*, brand:brands(*), stock_items:stock_items(*)').eq('org_id', orgId)
    if (branchId) query = query.eq('branch_id', branchId)
    const { data, error } = await query.order('name', { ascending: true })
    if (error) throw error
  ...
  async getProduct(productId: string): Promise<ProductWithStock | null> {
    const { data, error } = await supabase.from('products').select('*, brand:brands(*), stock_items:stock_items(*)').eq('id', productId).maybeSingle()
    if (error) throw error
  ...
  async getStockItems(productId: string): Promise<StockItem[]> {
    const { data, error } = await supabase.from('stock_items').select('*').eq('product_id', productId).order('expiry_date', { ascending: true })
    if (error) throw error
    return (data as StockItem[]) || []
  }

  async createStockItem(data: Partial<StockItem>): Promise<StockItem> {
    const { data: result, error } = await supabase.from('stock_items').insert(data).select().single()
    if (error) throw error
  ...
  async createImportTemplate(data: Partial<ImportTemplate>): Promise<ImportTemplate> {
    const { data: result, error } = await supabase.from('import_templates').insert(data).select().single()
    if (error) throw error
