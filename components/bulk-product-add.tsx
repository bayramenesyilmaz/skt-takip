    ...
    <Card className="border-primary/20">
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-muted-foreground">
          WhatsApp veya mesajdan kopyaladığınız ürünleri aşağıya yapıştırınız.
        </p>
      </CardHeader>
        ...
        {!isParsed ? (
          <>
            <div className="space-y-2">
              <label className="text-sm font-medium">Ürün Listesi</label>
              <textarea
                value={text}
...
Metin tahin+pekmez 290+380gr - 19/09/2027`}
              ...
              <p className="text-xs text-muted-foreground">
                Her ürün yeni satırda olmalı ve tarih formatı GG/AA/YYYY şeklinde olmalıdır.
              </p>
            </div>
          ...
          <>
            <div className="flex gap-2 flex-wrap">
              <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-600">
                ...
                {validCount} geçerli ürün
              </Badge>
              {invalidCount > 0 && (
                <Badge variant="secondary" className="bg-red-500/10 text-red-600">
                  <AlertCircle className="w-3 h-3 mr-1" />
                  {invalidCount} hatalı ürün
                </Badge>
              )}
            </div>
            ...
            {parsedProducts.length === 0 && (
              <div className="text-center py-6 text-muted-foreground">
                ...
                <p className="text-sm">Hiç ürün bulunamadı</p>
              </div>
            )}

            <div className="flex gap-2">
              <Button onClick={handleAdd} disabled={validCount === 0} className="flex-1">
                <Check className="w-4 h-4 mr-2" />
                {validCount} Ürün Ekle
              </Button>
              <Button
                variant="outline"
                onClick={() => {
