...
import { Package, ArrowLeft, Mail, Lock, User, Building2, Store, Smartphone, Check } from 'lucide-react'
...
const orgTypes: {
  ...
  {
    type: 'multi_branch',
    label: 'Çok Şubeli Firma',
    description: 'Birden fazla şubesi olan zincir marketler',
    icon: Building2,
  },
  {
    type: 'single_branch',
    label: 'Tek Şubeli Firma',
    description: 'Tek mağaza veya küçük işletme',
    icon: Store,
  },
]
...
function RegisterContent() {
  ...
  const [orgType, setOrgType] = useState<OrganizationType>(
    initialType === 'multi_branch' || initialType === 'single_branch' ? initialType : 'single_branch',
  )
          ...
          <CardHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <CardTitle className="text-xl">
                {step === 'type' ? 'Firma Tipi Seç' : 'Kayıt Ol'}
              </CardTitle>
            </div>
          </CardHeader>
          <CardContent>
            {step === 'type' ? (
                  ...
                  <Link href="/local">
                    <button className="w-full text-left p-4 rounded-xl border-2 border-border hover:border-primary/50 transition-all">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-foreground">Yerel Mod (Ücretsiz)</h3>
                          <p className="text-sm text-muted-foreground">
                            Veritabanı gerektirmez, veriler cihazınızda saklanır
                          </p>
                        </div>
                      </div>
                    </button>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="flex items-center gap-2 p-3 bg-primary/5 rounded-lg mb-2">
                  {orgType === 'multi_branch' ? (
                    <Building2 className="w-4 h-4 text-primary" />
                  ) : (
                    <Store className="w-4 h-4 text-primary" />
                  )}
                  <span className="text-sm font-medium text-foreground">
                    {orgType === 'multi_branch' ? 'Çok Şubeli Firma' : 'Tek Şubeli Firma'}
                  </span>
                  <button
                    type="button"
                ...
                <div className="space-y-2">
                  <Label htmlFor="orgName">Firma Adı</Label>
                  <Input
                    id="orgName"
                    value={orgName}
